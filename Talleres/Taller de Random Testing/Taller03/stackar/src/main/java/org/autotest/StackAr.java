...
public class StackAr {
	...
	public Object pop() throws IllegalStateException {
		...
		Object rv = this.top();
		elems[readIndex] = null; // No se ponian en null los elementos popeados
		readIndex--;
		return rv;
	}
	...
	public boolean repOK() {
		...
		if (readIndex <-1 || readIndex > elems.length) return false;

		for (int j = readIndex + 1; j < elems.length; ++j) {
			if (elems[j] != null) return false;
		}
